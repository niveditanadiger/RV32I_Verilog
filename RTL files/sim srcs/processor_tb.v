...
bus_arbiter bus_arbiter(.data_out(data_out),.address_out_31_2(address_out[31:2]),.Tx_Active(Tx_Active),.Tx_req(Tx_req),.Data_addr(Data_addr),.Data_mem_data(Data_mem_data),.Uart_data(Uart_data),.mem_write_ctrl(mem_write_ctrl),.Data_mem_write_ctrl(Data_mem_write_ctrl));
